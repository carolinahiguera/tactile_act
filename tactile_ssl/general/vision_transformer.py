...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        if pos_embed_fn == "sinusoidal":
            self.pos_embed = SinusoidalEmbed(
                [self.img_size, self.img_size], [self.patch_size, self.patch_size], embed_dim=self.embed_dim
            )
        elif (
    ...
    def output_shape(self):
        x = torch.zeros(1, 6, self.img_size, self.img_size)
        x = self.forward(x)
...
class VisionTransformerPredictor(VisionTransformer):
    ...
    def forward(self, x, context_masks, masks=None):
